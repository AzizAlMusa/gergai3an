...
/* ── Host auth: daily password ── */
const hostAuthPath = path.join(__dirname, "db", "host-auth.json");
let hostAuthConfig = { adminSecret: "gergai3an-master-2024", passwordOverride: null, salt: "trivia-night-pepper-7x" };
try { hostAuthConfig = JSON.parse(fs.readFileSync(hostAuthPath, "utf-8")); } catch {}
const ADMIN_SECRET = process.env.ADMIN_SECRET || hostAuthConfig.adminSecret;

const WORD_POOL = [
  "star", "moon", "lion", "wave", "fire", "jade", "bolt", "echo", "sage", "dusk",
  "hawk", "reef", "peak", "glow", "mint", "fern", "rose", "tide", "mist", "dawn",
  "ruby", "lynx", "iris", "palm", "haze", "lark", "vale", "cove", "opal", "pine"
];

function getDailyPassword() {
  if (hostAuthConfig.passwordOverride) return hostAuthConfig.passwordOverride;
  const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
  const hash = crypto.createHash("sha256").update(today + hostAuthConfig.salt).digest();
  const w1 = WORD_POOL[hash[0] % WORD_POOL.length];
  const w2 = WORD_POOL[hash[1] % WORD_POOL.length];
  const num = (hash[2] % 90) + 10; // 10-99
  return `${w1}-${w2}-${num}`;
}
...
app.post("/api/host-auth", (req, res) => {
  ...
  const today = getDailyPassword();
  if (String(password || "").trim().toLowerCase() === today.toLowerCase()) {
    return res.json({ ok: true });
  }
  return res.status(401).json({ ok: false, error: "Incorrect password." });
});

app.get("/api/admin/daily-password", (req, res) => {
  const secret = req.query.secret || req.headers["x-admin-secret"];
  if (secret !== ADMIN_SECRET) {
    return res.status(403).json({ ok: false, error: "Forbidden." });
  }
  res.json({ ok: true, password: getDailyPassword(), date: new Date().toISOString().slice(0, 10) });
});
